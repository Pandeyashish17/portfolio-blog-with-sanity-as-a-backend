...
const Index = ({ posts }) => {
  ...
  return (
    <>
      <HeadSection title="Ashish - Posts" />
      <section className="flex flex-row flex-wrap   pt-28 bg-[#121212] text-white gap-4">
        {posts.map((post) => {
          return (
            <Link key={post._id} href={`/posts/${post.slug.current}`}>
              <div
                className="transition-all duration-150 flex w-full px-4 py-6 md:w-1/2 lg:w-[30vw] "
                data-aos="zoom-in"
              >
