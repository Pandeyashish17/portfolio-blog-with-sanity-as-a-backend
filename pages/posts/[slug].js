...
const Post = ({ post }) => {
  ...
  return (
    <>
      <HeadSection title={`Ashish - ${post.slug.current}`} />
      <main className="bg-[#121212] text-white">
        ...
        <hr className="my-5 mx-auto max-w-lg border border-slate-300" />
        {submitted ? (
          <div className="my-10 mx-auto flex max-w-2xl flex-col  p-10 text-white">
          ...
          >
            <h3 className="text-sm text-slate-300">Enjoyed this article?</h3>
            ...
            <input
              type="submit"
              className="focus:shadow-outline cursor-pointer rounded-sm bg-slate-600 py-2 px-4 font-bold 
               shadow hover:bg-slate-400 focus:outline-none"
            />
          </form>
        )}
        <div className="mt-10 mx-auto flex max-w-2xl flex-col space-y-2 p-10 shadow shadow-slate-500 border-b-2 border-white">
          ...
          {post.comments.map((comment) => (
            <div key={comment._id}>
              <p>
                <span className="text-slate-300">{comment.name}: </span>
                {comment.comment}
              </p>
            </div>
          ))}
